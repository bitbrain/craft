...
/**
 * Data mapper for {@link de.bitbrain.craft.models.Goal}
 *
 * @author Miguel Gonzalez <miguel-gonzalez@gmx.de>
 * @since 1.0
 * @version 1.0
 */
@Mapper("de.bitbrain.craft.models.Goal")
public interface GoalMapper {

	@Select
	Collection<Goal> findAll();
	
	@Select(condition = "id = $1")
	Goal findById(int id);
	
	@Select(condition = "recipe_id = $1")
	Goal findByRecipeId(int id);
	
	@Insert
	boolean insert(Goal goal);
	
	@Insert
	boolean insert(Collection<Goal> goals);
	
	@Update
	boolean update(Goal goal);

	@Update
	boolean update(Collection<Goal> goals);
	
	@Delete
	boolean delete(Goal goal);
	
	@Delete
	boolean delete(Collection<Goal> goals);
