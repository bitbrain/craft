...
public class PlayerWidget extends Actor implements Fadeable {
  ...
  private void setProgress(final Progress progress) {
    ...
    final Tween tween =
        Tween.to(progressProvider, TweenType.VALUE.ordinal(), duration).ease(TweenEquations.easeOutQuad)
    ...
    if (progress.getLevel() == this.progress.getLevel()) {
      ...
      tween.setCallback(new TweenCallback() {
        ...
        public void onEvent(int type, BaseTween<?> source) {
          ...
          Tween.to(progressProvider, TweenType.VALUE.ordinal(), oldProgress * 1f).target(progress.getCurrentProgress())
